<?php
    ...
    if (isset($_POST["submit"])){
        ...
        if($company == ""){
            ...
            $update_data = mysqli_query($conn, "INSERT INTO `cable_tickets`(`id`, `cable_company_id`, `cable_car_no`, `departure_datetime`, `location_from`, `price`) VALUES ('','$company','$ticketNo','$date','$dLocation','$price')") or die('query failed');
...
<html>
    <head>
        <title>TicketKinee | Add Cable Car Ticket</title>
...
<header class="header">
    ...
    <form action="addCableCarTicket.php" method="POST">
        <div class="section-container">
                <h1 class="heading">Add <span>Cable Car Tickets</span> </h1>
        </div>
            ...
            <div class="container">
                ...
                <select name="company">
                    <option value=""> Enter Company</option>
                        <?php
                        $company = $conn->query("SELECT * FROM cable_company");
                        while($row = $company->fetch_assoc()):
                        ?>
                    <option value="<?php echo $row['company_name'] ?>" <?php echo isset($from_place) && $company == $row['id'] ? "selected" : '' ?>><?php echo $row['company_name']?></option>
                        <?php endwhile; ?>
                </select>
            </div>
            <div class="container">
                <label for="ticketNo">Enter Cable Car Number</label>
                <input type="text" placeholder="Enter Cable Car" name="ticketNo" required>
            </div>
            <div class="container">
            ...
            <div class="container">
                ...
                <select name="dLocation">
                    <option value=""> Enter Departure Location</option>
                        <?php
                        $dLocation = $conn->query("SELECT * FROM cable_station");
