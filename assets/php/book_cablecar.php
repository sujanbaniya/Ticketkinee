    ...
    <body>
        ...
        <table>
            ...
            echo " Number of Travellers:".$no_of_travellers."<br><br>";
                ...
                $result = $conn->query("SELECT * FROM cable_tickets WHERE location_from='$location_from' AND departure_datetime='$departure_datetime'");
                        while($row=$result->fetch_assoc()):
                            $airlines_id=$row['cable_company_id'];
                            $airlines_details = $conn->query("SELECT company_logo FROM cable_company WHERE id='$airlines_id'");
                            $airlines_data = $airlines_details->fetch_assoc();
                    ?>
                <tr>    
                <td><img src="<?php echo $airlines_data['company_logo']?>"></td>
                    <td><?php echo $row['location_from']?></td>
                    <td><?php echo $row['cable_car_no']?></td>
                    <td><?php echo $row['departure_datetime']?></td>
                    <td><?php echo $row['price']?></td>
                    <form method="post" action="eSewa_cable_transaction.php">
                    ...
                    <input text="text" name="cable_car_no" hidden value="<?php echo $row['cable_car_no']?>">
                    ...
                    $_SESSION['cable_car_no'] = $row['cable_car_no'];
                ...
                <?php if($row=$result->fetch_assoc()<1)
                    ...
                    <td>No Other tickets Available.</td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
